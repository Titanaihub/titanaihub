...
async function loadDashboard() {
  try {
    ...
    try {
      const remoteOverview = await fetchJson(`${API_BASE}/api/overview?v=2`);
    ...
    try {
      const btc = await fetchJson(`${API_BASE}/api/coin/btc?v=2`);
      coins.btc = { ...coins.btc, ...btc };
    } catch (err) {
      console.error("btc load failed:", err);
    }

    try {
      const eth = await fetchJson(`${API_BASE}/api/coin/eth?v=2`);
      coins.eth = { ...coins.eth, ...eth };
    } catch (err) {
      console.error("eth load failed:", err);
    }

    try {
      const bnb = await fetchJson(`${API_BASE}/api/coin/bnb?v=2`);
      coins.bnb = { ...coins.bnb, ...bnb };
    } catch (err) {
      console.error("bnb load failed:", err);
    }
    ...
    try {
      ...
      console.error("whales load failed:", err);
    }
    ...
    const pre = qs("rawSnapshot");
    if (pre) {
      pre.textContent = `Dashboard load error: ${err.message}`;
    }
  }
}
