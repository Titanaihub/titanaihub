...
app.get("/api/debug-version", (req, res) => {
  res.json({ version: "CHAT-ROUND2-V2" });
});
...
app.post("/api/chat", (req, res) => {
  ...
  const allMockCoins = global.loadMockCoinData ? global.loadMockCoinData() : {};

  const btc = (coins && coins.btc) ? coins.btc : (allMockCoins.btc || {});
  const eth = (coins && coins.eth) ? coins.eth : (allMockCoins.eth || {});
  const bnb = (coins && coins.bnb) ? coins.bnb : (allMockCoins.bnb || {});
