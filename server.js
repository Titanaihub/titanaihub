...
app.get("/api/debug-version", (req, res) => {
  res.json({ version: "CHAT-ROUND2-V3" });
});
...
app.post("/api/chat", (req, res) => {
  ...
  const allMockCoins = loadMockCoinData();

  const btc = coins.btc ? coins.btc : (allMockCoins.btc || {});
  const eth = coins.eth ? coins.eth : (allMockCoins.eth || {});
  const bnb = coins.bnb ? coins.bnb : (allMockCoins.bnb || {});
